...
public class AiGoToCoverState : AiStates
{
    ...
    public void Enter(AiAgent agent)
    {
        ...
        agent.navMeshAgent.stoppingDistance = agent.config.goToCoverStoppingDistance;
    ...
    public void Update(AiAgent agent)
    {
       // Debug.Log(nearestCover.gameObject.name);
        if (nearestCover != null)
        {
            ...
            {
                Vector3 hideDir;
                Vector3 dist = (agent.transform.position - nearestCover.transform.position).normalized;
               // Debug.Log(Vector3.Dot(agent.transform.forward, dist));
                if(Vector3.Dot(agent.transform.forward, dist) < -0f)
                {
                    hideDir = nearestCover.transform.position - agent.gameObject.transform.position;
                }
                else
                {
                    hideDir = agent.gameObject.transform.position - nearestCover.transform.position;
                }
                
                Vector3 hidePosition = nearestCover.transform.position + hideDir.normalized * 10f;

                Ray backray = new Ray(hidePosition, -hideDir.normalized * 10f);
