...
public class AiSensor : MonoBehaviour
{
    ...
    public float proximityHeight = 1.0f;
    ...
    void SetHeights()
    {
        detectHeight = GetColliderHeight(this.gameObject.GetComponent<Collider>()) + this.gameObject.transform.position.y;
        proximityHeight = GetColliderHeight(this.gameObject.GetComponent<Collider>()) + this.gameObject.transform.position.y;
        suspectHeight = GetColliderHeight(this.gameObject.GetComponent<Collider>()) + this.gameObject.transform.position.y;
    }
    void Scan()
    {
       // count = Physics.OverlapSphereNonAlloc(transform.position, detectDist, colliders, scanLayers, QueryTriggerInteraction.Collide);
        colliders = Physics.OverlapSphere(transform.position, detectDist, scanLayers, QueryTriggerInteraction.Collide);
        ...
        var nearestCover = coversCollided
            .Where(collider =>
            {
                if (agent.health.hitDir != null)
                {
                    ...
                    //Debug.Log("Name of Object" + collider.gameObject.name + "It has a distance of" + Vector3.Distance(transform.position, collider.transform.position) +"The dot product of the object & player" +  (Vector3.Dot(dist, agent.health.hitDir.normalized)));
                    return Vector3.Dot(dist, agent.health.hitDir.normalized) < 1.0f;
                }
                return false;
            })
    ...
    public bool IsInSight(GameObject obj)
    {
        ...
        if (dest.y < this.gameObject.transform.position.y || dest.y > detectHeight)
        {
            return false;
        }
    ...
    public bool IsInProximity(GameObject obj)
    {
        ...
        Vector3 dest = obj.transform.position;
        Vector3 dir = dest - origin;
        if (dest.y < this.gameObject.transform.position.y || dest.y > proximityHeight)
        {
            return false;
        }
        ...
        if (ObstuctionCheck(obj, proximityHeight))
        {
            return false;
        }
    ...
    public bool IsSoundClose(Vector3 soundLocation)
    {
        ...
        if(dest.y < this.gameObject.transform.position.y || dest.y > suspectHeight)
        {
            return false;
        }
    ...
    bool ObstuctionCheck(GameObject obj, float scannerHeight)
    {
        ...
        dest.y = GetColliderHeight(obj.GetComponent<Collider>()) + obj.transform.position.y;
    ...
    Mesh CreateProximityMesh()
    {
        ...
        Vector3 topCenter = bottomCenter + Vector3.up * proximityHeight;
        Vector3 topLeft = bottomLeft + Vector3.up * proximityHeight;
        Vector3 topRight = bottomRight + Vector3.up * proximityHeight;
        ...
        for (int i = 0; i < segments; ++i)
        {
            ...
            topLeft = bottomLeft + Vector3.up * proximityHeight;
            topRight = bottomRight + Vector3.up * proximityHeight;
